...
def process_data_from_azure() -> str:
    ...
    # 1. Fetch Orders (latest matching file)
    orders_stream, orders_name = _get_latest_blob(container, 'Orders*.xlsx')
    log(f"Reading Orders from Azure: {orders_name}")
    df = pd.read_excel(orders_stream)
